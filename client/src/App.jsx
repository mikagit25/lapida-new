          ...
          <div className="flex items-center space-x-4">
            {isAuthenticated ? (
              <>
                <NotificationBell />
                {user?.avatar ? (
                  <AsyncImage url={user.avatar} alt="Avatar" className="w-8 h-8 rounded-full object-cover border border-gray-300" />
                ) : (
...
// Главная страница и мемориалы вынесены отдельно для чистоты
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h1 className="text-4xl font-extrabold text-gray-900 sm:text-5xl md:text-6xl">Lapida</h1>
          <p className="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">Цифровая платформа для создания и поддержания памяти о близких людях</p>
          <div className="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8 space-y-3 sm:space-y-0 sm:space-x-3">
            <Link to="/memorials" className="w-full sm:w-auto flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10">Просмотреть мемориалы</Link>
            {isAuthenticated ? (
              <Link to="/create-memorial" className="w-full sm:w-auto flex items-center justify-center px-8 py-3 border border-indigo-600 text-base font-medium rounded-md text-indigo-600 bg-white hover:bg-indigo-50 md:py-4 md:text-lg md:px-10">Создать мемориал</Link>
            ) : (
              <Link to="/register" className="w-full sm:w-auto flex items-center justify-center px-8 py-3 border border-indigo-600 text-base font-medium rounded-md text-indigo-600 bg-white hover:bg-indigo-50 md:py-4 md:text-lg md:px-10">Зарегистрироваться</Link>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mb-6">Сохраните память о близких и поделитесь их историями</p>
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1">
              <input type="text" placeholder="Поиск по имени..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        {sortedMemorials.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">{searchTerm ? 'Мемориалы не найдены' : 'Пока нет мемориалов'}</h3>
            <p className="text-gray-600 mb-4">{searchTerm ? 'Попробуйте изменить поисковый запрос' : 'Станьте первым, кто создаст мемориал'}</p>
            {!searchTerm && (
              <Link to="/create-memorial" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">Создать мемориал</Link>
            )}
          </div>
        ) : (
              ...
              <div key={memorial._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                <div className="relative">
                  {memorial.profileImage ? (
                    <AsyncImage url={memorial.profileImage} alt={memorial.fullName} className="w-full h-48 object-cover" onError={e => {
